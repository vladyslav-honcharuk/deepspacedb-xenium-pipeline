"""H&E image alignment and transformation helper based on scikit-image and tifffile.

Provides affine transformation, warping, and multi-resolution pyramid generation for
H&E histology images aligned with Xenium spatial coordinate frames.
"""
...
def _write_pyramid_tiff(path: str, image: np.ndarray, bigtiff: bool) -> None:
    ...
    write_opts: Dict[str, Any] = dict(compression="deflate", tile=(tile, tile), photometric=photometric)
    ...
    with TiffWriter(path, bigtiff=bigtiff) as tif:
        ...
        for i in range(n_levels):
            kwargs: Dict[str, Any] = {"subifds": n_levels - 1} if i == 0 else {"subfiletype": 1}
...
def generate_he_pngs(
    ...
    pd.DataFrame([extent_microns], columns=["min_x", "max_x", "min_y", "max_y"]).to_csv(extent_file, index=False)
...
def _resolve_levels(he_image_path: str, pyramid_level: int):
    ...
    with TiffFile(he_image_path) as tif:
        ...
        if level0_page is None:
            raise ValueError("Could not read the level-0 page from the H&E TIFF")
...
def transform_he_auto(
    ...
    return transform_he_skimage(
        ...
        png_dpis=png_dpis,
        png_max_dimension=png_max_dimension,
