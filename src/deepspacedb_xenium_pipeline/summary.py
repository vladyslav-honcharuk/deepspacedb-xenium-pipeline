...
_CSV_FIELDS = [
    "sample_name", "sample_path", "processing_timestamp", "files_in_raw", "files_in_processed",
    "has_raw_dir", "has_experiment_file", "has_cell_feature_matrix", "has_transcripts",
    "has_cells_parquet", "has_cell_boundaries", "has_nucleus_boundaries", "has_morphology",
    "has_morphology_mip", "has_morphology_focus", "has_he_image", "has_gene_panel",
    "has_matrix_mtx", "has_barcodes", "has_features", "has_cells_zarr", "has_transcripts_zarr",
    "raw_processing_success", "raw_processing_error", "spatialdata_success", "spatialdata_error",
    "transcriptomics_success", "transcriptomics_error", "complete_processing_success",
    "complete_processing_error", "has_processed_dir", "has_spatialdata_zarr",
    "has_final_processed_zarr", "has_images", "has_bins", "has_single_cell_exports",
]

_OUTPUT_FLAG_FIELDS = [
    "has_processed_dir", "has_spatialdata_zarr", "has_final_processed_zarr",
    "has_images", "has_bins", "has_single_cell_exports", "files_in_processed",
]
...
class SampleAnalyzer:
    ...
    def _scan_raw(self, raw_dir: Path, summary: SampleSummary) -> None:
        ...
        summary.has_experiment_file = any_match(
            lambda n: "experiment" in n and (".xenium" in n or ".json" in n)
        )
        summary.has_he_image = any_match(
            lambda n: ("he" in n and ".tif" in n) or ("h&e" in n and ".tif" in n)
        )
        summary.has_cell_feature_matrix = any_match(
            lambda n: "cell_feature_matrix" in n and (".h5" in n or ".zarr" in n)
        )
        summary.has_transcripts = any_match(
            lambda n: "transcript" in n and (".parquet" in n or ".csv" in n)
        )
        summary.has_cells_parquet = any_match(
            lambda n: "cells" in n and (".parquet" in n or ".csv" in n) and "boundaries" not in n
        )
        summary.has_cell_boundaries = any_match(
            lambda n: "cell_boundaries" in n or "cell_boundary" in n
        )
        summary.has_nucleus_boundaries = any_match(
            lambda n: "nucleus_boundaries" in n or "nucleus_boundary" in n
        )
        summary.has_morphology = any_match(
            lambda n: "morphology" in n and ".tif" in n and "mip" not in n and "focus" not in n
        )
        summary.has_morphology_mip = any_match(
            lambda n: "morphology_mip" in n or ("morphology" in n and "mip" in n)
        )
    ...
    with open(output_file, "w", newline="") as csvfile:
        ...
        for summary in summaries:
            ...
            for field_name in _CSV_FIELDS:
                ...
                row[field_name] = ("Yes" if value else "No") if isinstance(value, bool) else value
            writer.writerow(row)  # one row per summary (original wrote only the last)
