...
Xenium records and returns a tidy ``platform, series, sample`` table. Refactored
from the original ``xenium_collector.py``: it logs through the package logger
instead of ``print``, takes rate limits from :class:`PipelineConfig`, and raises
:class:`CollectionError` for unrecoverable failures instead of returning bare
sentinels.
"""
...
class SampleCollector:
    """Collect Xenium GSM samples from NCBI GEO via the E-utilities API."""
    ...
    def collect(self) -> pd.DataFrame:
        ...
        self.logger.info("Starting Xenium sample collection")
        ...
        self.logger.info("Collected %d total samples", len(df))
        return df

    @staticmethod
    def save(df: pd.DataFrame, output: str = "xenium_samples.csv") -> Optional[str]:
        """Write the collected table to CSV. Returns the path, or None if empty."""
        if df.empty:
            return None
    ...
    def get_xenium_platforms(self) -> List[str]:
        ...
        if root is None:
            return []
        accessions = [
            doc.find("Accession").text
            for doc in root.findall(".//DocumentSummary")
            if doc.find("Accession") is not None
        ]
    ...
    def get_samples_for_platform(self, platform: str) -> List[Dict]:
        ...
        for doc in root.findall(".//DocumentSummary"):
            ...
            if sample_acc is not None and gse_elem is not None:
                series_num = gse_elem.text.split(";")[0].strip()
                samples.append(
                    {"platform": platform, "series": f"GSE{series_num}", "sample": sample_acc.text}
                )
