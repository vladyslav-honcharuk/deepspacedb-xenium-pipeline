...
class CellsParquetBuilder:
    ...
    def _load_geometry(self, processed_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if cell_bnds.exists() or nuc_bnds.exists():
            ...
            if nuc_poly_counts:
                nuc_count_df = pd.Series(nuc_poly_counts, name="nucleus_count", dtype="Int64").to_frame()
            else:
                nuc_count_df = empty
    ...
    def _agg_boundaries(self, boundary_file: Path, kind: str) -> Tuple[pd.DataFrame, Dict]:
        empty = pd.DataFrame({"cell_id": []}).set_index("cell_id")
        try:
            df = pd.read_parquet(boundary_file, columns=["cell_id", "vertex_x", "vertex_y", "label_id"])
        except Exception as exc:  # noqa: BLE001
                    ...
                    lambda t: pd.Series(
                        {
                            ...
                            "x_centroid": np.average(t["cx"], weights=t["area"]) if t["area"].sum() > 0 else np.nan,
                            "y_centroid": np.average(t["cy"], weights=t["area"]) if t["area"].sum() > 0 else np.nan,
                        }
                    )
                )
                .reset_index()
    ...
    def _transcript_centroids(self, processed_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            if df.empty:
                return empty, empty
            centroids = df.groupby("cell_id").agg(x_centroid=("x_location", "mean"), y_centroid=("y_location", "mean"))
