"""Build SpatialData ``TableModel`` + cell-circle ``ShapesModel`` from disk.

Three entry points mirror the three matrix formats Xenium exports ship in:

* :meth:`TableLoader.from_h5`   - ``cell_feature_matrix.h5``
* :meth:`TableLoader.from_zarr` - ``cell_feature_matrix.zarr.zip``
* :meth:`TableLoader.from_mtx`  - 10x MEX (``matrix.mtx.gz`` + tsv files)

Each handles the messy realities of real-world data: integer-vs-string cell IDs,
missing ``cells.parquet`` (fall back to zarr, then boundaries, then transcript
centroids), control-probe filtering, and positional alignment when IDs cannot be
matched. Behaviour is preserved verbatim from the original implementation.
"""
...
class TableLoader:
    ...
    def load_10x_fixed(self, path: Path):
        ...
        features_file = next(
            (path / p for p in ("features.tsv.gz", "features.tsv") if (path / p).exists()), None
        )
        if features_file is not None:
            self.fix_features_file(features_file)

        barcodes_file = next(
            (path / p for p in ("barcodes.tsv.gz", "barcodes.tsv") if (path / p).exists()), None
        )
    ...
    def fix_features_file(self, features_file: Path) -> None:
        ...
        try:
            ...
            if n_cols == 1:
                fixed = pd.DataFrame(
                    {0: features_df.iloc[:, 0], 1: features_df.iloc[:, 0], 2: "Gene Expression"}
                )
            elif n_cols == 2:
                fixed = pd.DataFrame(
                    {0: features_df.iloc[:, 0], 1: features_df.iloc[:, 1], 2: "Gene Expression"}
                )
            else:
                return
    ...
    def from_h5(self, processed_dir: Path, specs: Dict[str, Any]) -> Tuple[Any, Any]:
        ...
        adata = sc.read_10x_h5(processed_dir / constants.CELL_FEATURE_MATRIX_H5, gex_only=False)
    ...
    def _radii_for(self, adata, keys, nucleus_bnd: Path, cell_bnd: Path) -> np.ndarray:
        ...
        boundary_file = (
            nucleus_bnd if nucleus_bnd.exists() else (cell_bnd if cell_bnd.exists() else None)
        )
        if boundary_file is not None:
            ...
            return np.array(
                [np.sqrt(areas.get(str(cid), 100.0) / np.pi) for cid in adata.obs[keys.CELL_ID]]
            )
    ...
    def _read_metadata(self, processed_dir, cells_parquet, cells_zarr, nucleus_bnd, cell_bnd):
        ...
        if boundary_file.exists():
            ...
            centroids = df.groupby(keys.CELL_ID).agg(
                {"vertex_x": "mean", "vertex_y": "mean"}
            ).reset_index()
            return pd.DataFrame(
                {
        ...
        raise FileNotFoundError(
            f"No cell metadata source in {processed_dir}: tried cells.parquet, "
            f"cells.zarr.zip, and boundary files"
        )
    ...
    def _metadata_from_zarr(self, cells_zarr: Path) -> pd.DataFrame:
        ...
        try:
            ...
            cell_ids = root["cell_id"][:, 0].copy().astype(str)
            ...
            return metadata.rename(
                columns={"cell_centroid_x": "x_centroid", "cell_centroid_y": "y_centroid"}
            )
        finally:
            store.close()
    ...
    def from_zarr(self, processed_dir: Path, specs: Dict[str, Any]) -> Tuple[Any, Any]:
        ...
        adata.obs_names = cf["cell_id"][:, 0].copy().astype(str)
        store.close()
        ...
        metadata[keys.CELL_ID] = metadata_cell_ids
        if (
            len(set(metadata_cell_ids) & set(adata.obs_names.astype(str))) == 0
            and len(metadata) == len(adata)
        ):
    ...
    def _read_zarr_metadata(self, processed_dir: Path, adata) -> Tuple[pd.DataFrame, Any]:
        ...
        if not transcripts_file.exists():
            raise FileNotFoundError(
                f"Neither {cells_parquet}, {cells_zarr}, nor {transcripts_file} found"
            )
        ...
        centroids = df.groupby("cell_id").agg(
            {"x_location": "mean", "y_location": "mean"}
        ).reset_index().rename(columns={"x_location": "x_centroid", "y_location": "y_centroid"})
        ...
        if len(metadata) != len(adata):
            self.logger.warning(
                "Transcripts has %d cells but expression has %d", len(metadata), len(adata)
            )
            if len(metadata) < len(adata):
    ...
    def from_mtx(
        self, path: Path, cells_as_circles: bool, specs: Dict[str, Any]
        ...
        np.testing.assert_array_equal(
            metadata.cell_id.astype(str), adata.obs_names.astype(str)
        )
        ...
        table = TableModel.parse(
            adata, region=specs["region"], region_key="region", instance_key=str(keys.CELL_ID)
        )
