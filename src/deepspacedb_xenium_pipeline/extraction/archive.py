...
try:
    import zipfile_deflate64  # noqa: F401 - patches zipfile for compress_type 9
except ImportError:
...
class ArchiveExtractor:
    ...
    def is_archive(file_path: Path) -> bool:
        ...
        if name.endswith(".zarr.zip"):
            ...
            return False
        return name.endswith(".zip") or name.endswith(".tar.gz") or name.endswith(".tar")
    ...
    def _resolve_outs_priority(self, all_files: List[Path]) -> Set[Path]:
        ...
        if outs and xe_outs:
            self.logger.info(
                "Both _outs.zip and xe_outs.zip present; prioritizing _outs.zip (%s) and skipping %s",
    ...
    def _extract_tar_with_mode(self, archive_path: Path, temp_dir: Path, mode: str) -> int:
        extracted = 0
        # tarfile.open's stubs only accept literal mode strings, not a runtime str.
        with tarfile.open(archive_path, mode) as archive:  # type: ignore[call-overload]
    ...
    def _extract_zip(self, archive_path: Path, temp_dir: Path) -> int:
        if zipfile_deflate64 is None:
            self.logger.debug("zipfile_deflate64 is not installed; Deflate64 (compress_type 9) zips may fail")
        extracted = 0
