...
class FileOrganizer:
    ...
    def _organize_morphology_focus(
        self, temp_dir: Path, processed_dir: Path, all_temp_files: List[Path]
    ) -> int:
        """Materialize focus planes and a single tiled ``morphology_focus.ome.tif``."""
        if self.config.skip_focus_copy:
            self.logger.info("Skipping focus copy (skip_focus_copy=True)")
            return 0
