...
Every shared magic value lives here exactly once. In particular, the Xenium
pixel size has a single source of truth.
"""
...
ESSENTIAL_FILES: Dict[str, List[str]] = {
    ...
    "he_image.ome.tif": [
        # Patterns kept for documentation; matching uses token-based
        # :func:`find_he_image_file` instead of these globs,
        # which false-positive on marker names containing "he" (cadherin, HER2).
        "*H&E*.tif*",
        "*_HE_*.tif*",
        "*_he_*.tif*",
        "*HE.tif*",
...
# Older analyzer runs without ``codeword_category``: exclude these transcript
# feature names when reconstructing counts from transcripts.parquet.
TRANSCRIPT_CONTROL_PATTERNS: List[str] = [
    "NegControlProbe_",
    "NegControlCodeword_",
    "BLANK_",
    "UnassignedCodeword_",
...
def read_pixel_size(experiment_file: Path) -> float:
    ...
    It never raises: a missing or malformed file yields
