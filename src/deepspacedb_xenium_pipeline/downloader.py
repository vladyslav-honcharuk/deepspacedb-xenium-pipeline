...
class SampleDownloader:
    ...
    def download_gsm_sample(
        ...
        try:
            ...
            if self._is_downloaded(sample_dir):
                self.logger.info("%s already downloaded; skipping", gsm_id)
                return DownloadResult(sample_path=sample_dir, success=True, gsm_id=gsm_id, raw_path=sample_dir / "raw")
            ...
            result = DownloadResult(
                ...
                success=True,
                gsm_id=gsm_id,
                raw_path=sample_dir / "raw",
                files_processed=[f.name for f in downloaded],
            )
    ...
    def download_multiple(self, samples: List[Tuple[str, Optional[str], Optional[str]]]) -> Dict[str, DownloadResult]:
